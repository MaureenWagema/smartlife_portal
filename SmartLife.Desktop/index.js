...
function load_application() {
    $(function () {
        ...
        SmartLife.app.on("resolveLayoutController", function (args) {
            ...
            if (args.viewInfo.viewName == 'client_register') {
                args.layoutController = findController('empty', args.availableLayoutControllers);
            }
            if (args.viewInfo.viewName == 'client_forgot') {
                args.layoutController = findController('empty', args.availableLayoutControllers);
            }
            if (args.viewInfo.viewName == 'broker_register') {
                args.layoutController = findController('empty', args.availableLayoutControllers);
            }
            if (args.viewInfo.viewName == 'broker_forgot') {
                args.layoutController = findController('empty', args.availableLayoutControllers);
            }
            if (args.viewInfo.viewName == 'group_register') {
                args.layoutController = findController('empty', args.availableLayoutControllers);
            }
            if (args.viewInfo.viewName == 'group_forgot') {
                args.layoutController = findController('empty', args.availableLayoutControllers);
            }
			...
			//application_form
            if (args.viewInfo.viewName == 'application_form' && SmartLife.is_logged == 0 || SmartLife.is_logged == undefined) {
                args.layoutController = findController('empty', args.availableLayoutControllers);
            }
            ...
            if (args.viewInfo.viewName == 'request_claim_form' && SmartLife.is_logged == 0 || SmartLife.is_logged == undefined) {
                args.layoutController = findController('empty', args.availableLayoutControllers);
            }
        ...
        SmartLife.app.on("navigating", function (e) {
            try {
                ...
                if (expose_n != undefined && expose_n == 1) {
                    ...
                    if (SmartLife.is_logged != 1) {
                        if (e.uri == "login" || e.uri == "brokers" || e.uri == "bancassurance" || e.uri == "register" ||
                            ...
                            e.uri == "broker_forgot" || e.uri == "broker_register" || e.uri == "client_forgot" ||
                            e.uri == "client_register" || e.uri == "group_register" || e.uri == "group_forgot") {
                            //do nothing
                        } else {
                            e.uri = startupView;
                        }
                ...
                //,
                if (view == "login" || view == "brokers" || view == "bancassurance" || view == "register" || view == "user_login" ||
                    view == "agent_login" || view == "creditLife" || view == "group_login" || view == "member_login" || view == "confrim_password" ||
                    view == "forgot_password" || view == "broker_forgot" || view == "broker_register" || view == "client_forgot" ||
                    view == "client_register" || view == "group_register" || view == "group_forgot" ||
                    ((view == "application_form" || view == "reports" || view == "proposal_form" || view == "request_claim_form" || view == "policy_det" || view == "get_quotation") && (SmartLife.is_logged == 0 || SmartLife.is_logged == undefined))) {
...
function sign_in_usr() {
    ...
    logged_user.byKey(1).done(function (values) {
        ...
        SmartLife.agent_position = values.agent_position;
        SmartLife.agent_position_id = values.agent_position_id;
        SmartLife.agent_office = values.agent_office;
        SmartLife.IsManager = values.IsManager;
        ...
        SmartLife.IsCreditLifeUser = values.IsCreditLifeUser;
...
$(function () {
	...
	if (window.location.host == "smartlife.glicolife.com" && getProtocolWithoutHostname() == "https") {
        ...
        SmartLife.url = getProtocolWithoutHostname()+"://"+window.location.hostname+":97/glico_api/public/";
		//SmartLife.rpt_url = "http://197.159.142.171:85/api/Report/";
    ...
    window.addEventListener("mousemove", resetTimer); // Mouse move
    window.addEventListener("keydown", resetTimer); // Keyboard keypress
...
let inactivityTimer; // Variable to store the timeout ID
// Function to show an alert after 2 minutes of inactivity
function showAlert() {
    ...
    if (SmartLife.login_type == 1) {
        ...
        //SmartLife.app.router.register(":view/:id", { view: "login", id: undefined });
        SmartLife.app.navigate('login', { root: true });
    }
    if (SmartLife.login_type == 2) {
        SmartLife.login_type = 0;
        //SmartLife.app.router.register(":view/:id", { view: "agent_login", id: undefined });
        if (SmartLife.channel == 4 || SmartLife.channel == "4") {
            SmartLife.app.navigate('bancassurance', { root: true });
        } else {
            SmartLife.app.navigate('agent_login', { root: true });
        }
    }
    if (SmartLife.login_type == 3) {
        //alert(SmartLife.login_type);
        SmartLife.login_type = 0;
        //SmartLife.app.router.register(":view/:id", { view: "user_login", id: undefined });
        SmartLife.app.navigate('user_login', { root: true });
    }
    if (SmartLife.login_type == 4) {
        SmartLife.login_type = 0;
        //SmartLife.app.router.register(":view/:id", { view: "group_login", id: undefined });
        SmartLife.app.navigate('group_login', { root: true });
    }
    if (SmartLife.login_type == 5) {
        SmartLife.login_type = 0;
        //SmartLife.app.router.register(":view/:id", { view: "brokers", id: undefined });
...
function resetTimer() {
    ...
    inactivityTimer = setTimeout(showAlert, 50 * 60 * 1000); // 2 minutes in milliseconds
}
