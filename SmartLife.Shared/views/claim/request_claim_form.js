SmartLife.request_claim_form = function (params) {
    ...
    var SubmitData;
    var IsComplete = true;
    var vs_claim_type = false;
    var vs_insured_details = true;
    var vs_claim_details = true;
    var loan_keys;
    var plan_code;
    ...
    if (get_data['plan_code'] != undefined) plan_code = get_data['plan_code'];
    ...
    if (get_data['claim_type'] != undefined) {
        //prem_units = get_data['prem_units'];
        vs_claim_type = false;
        vs_insured_details = false;
        vs_claim_details = false;
        if (get_data['claim_type'] == "RFD") vs_refund_bank = true;
        if (get_data['claim_type'] == "RFU") vs_refund_emp = true;
    }
    //TODO - If its wongful, then give property is wrongful and display only the wrongful claims...
    var is_wrongful = 0;
    if (get_data['is_wrongful'] != undefined) {
        is_wrongful = get_data['is_wrongful'];
    }
    

    var ClaimType = [];//SmartLife.ClaimType;
    IDTypes = SmartLife.IDTypes.filter(id => (id.ShowInMProposal == 1));
    ...
    if (SmartLife.login_type == 3) {
        ...
        if (SmartLife.pos_type == 2) {//micro
            ...
            ClaimType = SmartLife.ClaimType.filter(claim => (claim.claim_type != "RFD" && claim.claim_type != "RFU" && claim.claim_type != "REP" &&
                (claim.AffectGroupLife == "0" || claim.AffectAll == "1")));
            //ClaimType = SmartLife.ClaimType.filter(claim => (claim.AffectGroupLife == "0"));  && claim.claim_type != "RFU"
            //claim.AffectGroupLife == "0" || claim.AffectAll == "1" && 
            if (is_wrongful == "1" || is_wrongful == 1 || get_data['claim_type'] == "RFD" || get_data['claim_type'] == "RFU") {
                
                //alert("RFD & RFU");// && claim.claim_type == "RFU"
                ClaimType = SmartLife.ClaimType.filter(claim => (claim.claim_type == "RFD" || claim.claim_type == "RFU" || claim.claim_type == "RFDLOAN" || claim.claim_type == "RFULOAN"));
            } else if (get_data['claim_type'] == "REP") {
                //alert("RFD & RFU");// && claim.claim_type == "RFU"
                ClaimType = SmartLife.ClaimType.filter(claim => (claim.claim_type == "REP"));
            }
        }
    ...
    //mode.payment_mode == "2" || IsForClaims
    ...
    var PayMethods = SmartLife.Paymentinfo.filter(mode => (mode.IsForClaims == "1"));
    if (get_data['claim_type'] == "RFD" || get_data['claim_type'] == "RFU") {
        PayMethods = SmartLife.Paymentinfo.filter(mode => (mode.IsForClaims == "1" || mode.IsCash == "1"));
    }
    var LoanPayMethods = SmartLife.Paymentinfo.filter(mode => (mode.IsForClaims == "1" || mode.IsCheckoff == "1"));

    var ClaimBankBranches = [];
    var LoanBankBranches = [];
    ...
    var viewModel = {
        ...
        getClaimTypeGroup: function () {
            viewModel.LoadPanelShown(true);
            var url = "claims/getClaimTypeGroup";
            var get_form = new DB({
                name: "get claim group types"
            });
            get_form.DBget(url).done(function (result) {
                viewModel.LoadPanelShown(false);
                if (result.success == true) {
                    console.log(result.ClaimType);
        ...
        getClientPolicy: function (show_policy) {
            ...
            let params = "?policy_no=" + policy_no;
            if (SmartLife.login_type == 1) {
                params = "?search_entry=" + SmartLife.clientno +"&criteria=6";
            }
            get_form.DBget(default_url + params).done(function (result) {

                if (result.success == true) {
                    viewModel.policy_obj(result.PolicyDetails);
                    ClientPolicies = result.PolicyDetails;
                    plan_code = result.PolicyDetails[0]['plan_code'];
                    //if 
                    ...
                    if (SmartLife.planinfo[index]['Funeral_cover'] == 1) {
                        ...
                        //check for investment premium to greator than zero else remove
                        if (parseFloat(viewModel.policy_obj()[0].investment_prem) > 0) {
                            //do nothing
                        } else {
                            //alert("here");
                            //remove PWD and LON
                            let tmpClaimType = ClaimType.filter(claim => (claim.isLoan != "1" && claim.IsPwd != "1"));
                    ...
                    if (SmartLife.login_type == 3) formClaimInstance.updateData("PolicyId", result.PolicyDetails[0]['id']);
                    ...
                    if (formClaimInstance.getEditor("CurrentCashValue") != null && claim_type == "PWD") {
                        ...
                        if (is_micro == 1) {
                            ...
                            viewModel.getMicroCashValue();
                        } else {
                            viewModel.getMicroCashValue();
                            /*let cash_value = result.PolicyDetails[0]['cashvalue'];
                            if (cash_value == null || cash_value == undefined || cash_value == '') {
                                cash_value = 0;
                            }
                            if (formClaimInstance.getEditor("CurrentCashValue") != null)
                                formClaimInstance.updateData("CurrentCashValue", result.PolicyDetails[0]['cashvalue']);
                            if (formClaimInstance.getEditor("TotalCashValue") != null)
                                formClaimInstance.updateData("TotalCashValue", result.PolicyDetails[0]['cashvalue']);
                            if (formClaimInstance.getEditor("TotalLoanValue") != null)
                                formClaimInstance.updateData("TotalLoanValue", result.PolicyDetails[0]['cashvalue']); */                           
                        }
                    }



                    if (claim_type != "DTH" && claim_type != "HCI") {
                        //alert("here");
                    ...
                    //alert("am here");
                    if (data['claim_type'] == "DTH" || data['claim_type'] == "HCI") {
        ...
        viewShown: function () {
            ...
            if (get_data['claim_type'] == "RFD" || get_data['claim_type'] == "RFU") {
                formClaimInstance.updateData("claim_type", get_data['claim_type']);
                if (get_data['claim_type'] == "RFD") {
                    formClaimSubmitInstance.updateData("BankCode", get_data['bank_code']);
                    formClaimSubmitInstance.updateData("ReferenceNumber", get_data['reference_no']);
                } else if (get_data['claim_type'] == "RFD") {
                    formClaimSubmitInstance.updateData("Emp_code", get_data['emp_code']);
                    formClaimSubmitInstance.updateData("ReferenceNumber", get_data['reference_no']);
                }
            } else if (get_data['claim_type'] == "REP") {
                formClaimInstance.updateData("claim_type", get_data['claim_type']);
                formClaimInstance.updateData("ProposalNumber", proposal_no);
            } else {
                //viewModel.LoadPanelShown(true);
                if (SmartLife.login_type == 3) {//POS
                   ...
                   // }
                } else if (SmartLife.login_type == 4) {//group
                    viewModel.getClaimTypeGroup();
                }
            }
        },
        ...
        getMicroCashValue: function () {
            viewModel.LoadPanelShown(true);
            var get_form = new DB({
                name: "get the micro cashvalue"
            });
            let url = "policy/getMicroCashValue?policy_no=" + policy_no;
            get_form.DBget(url).done(function (result) {
                viewModel.LoadPanelShown(false);
                if (result.success == true) {
        ...
        default_form: function () {
            is_send_OTP = true;//on go to production set to true
            vs_refund_bank = false;
            vs_refund_emp = false;
            formClaimInstance.itemOption("PolicyId", "visible", false);//
            formClaimInstance.itemOption("ProposalNumber", "visible", false);
            ...
            formClaimInstance.itemOption("CLAIM DETAILS.TotalLoanValue", "visible", false);
            formClaimInstance.itemOption("CLAIM DETAILS.TotalLoanAvailable", "visible", false);
            formClaimInstance.itemOption("CLAIM DETAILS.TotalWithdrawal", "visible", false);

            formClaimInstance.itemOption("CLAIM DETAILS.btnFetchPolicy", "visible", false);
            formClaimInstance.itemOption("CLAIM DETAILS.LoanPolicies", "visible", false);
            ...
            formClaimSubmitInstance.itemOption("PaySourceType", "visible", false);
            formClaimSubmitInstance.itemOption("btncomplete", "visible", true);
            //formClaimSubmitInstance.itemOption("ClaimDefaultPay_method", "visible", false);
            //formClaimSubmitInstance.itemOption("attachements", "visible", false);
            formClaimSubmitInstance.itemOption("ReferenceNumber", "visible", false);

            //WrongfulBank,ReferenceNumber,WrongfulEmployer,ReferenceNumber,WrongfulDateFrom,WrongfulDateTo
            formClaimSubmitInstance.itemOption("Emp_code", "visible", false);
            formClaimSubmitInstance.itemOption("BankCode", "visible", false);
            formClaimSubmitInstance.itemOption("ReferenceNumber", "visible", false);
            formClaimSubmitInstance.itemOption("DateFrom", "visible", false);
            formClaimSubmitInstance.itemOption("DateTo", "visible", false);
        },
        ...
        refresh_dataSources: function () {
            ...
            if (formClaimInstance.getEditor("claim_type") != null)
                formClaimInstance.getEditor("claim_type").option("dataSource", ClaimType);
            if (formClaimInstance.getEditor("PolicyId") != null)
                formClaimInstance.getEditor("PolicyId").option("dataSource", ClientPolicies);
            ...
            //claim_bank branches
            if (formClaimSubmitInstance.getEditor("ClaimDefaultEFTBankBranchCode") != null)
                formClaimSubmitInstance.getEditor("ClaimDefaultEFTBankBranchCode").option("dataSource", ClaimBankBranches);
            if (formClaimSubmitInstance.getEditor("LoanDefaultEFTBankBranchCode") != null)
                formClaimSubmitInstance.getEditor("LoanDefaultEFTBankBranchCode").option("dataSource", LoanBankBranches);
            if (formClaimSubmitInstance.getEditor("PaySourceRawData") != null)
                formClaimSubmitInstance.getEditor("PaySourceRawData").option("dataSource", PaySourceRawDataStore);
        },
        ...
        claimTypeChanged: function (value) {
            ...
            if (value == "PWD" || value == "LON") {//partial and loan
                ...
                if (value == "LON") {
                    ...
                    //TotalWithdrawal
                    formClaimInstance.itemOption("CLAIM DETAILS.TotalLoanValue", "visible", true);
                    formClaimInstance.itemOption("CLAIM DETAILS.TotalLoanAvailable", "visible", true);
                    ...
                    if (is_micro == 1) {
                        ...
                        formClaimInstance.itemOption("CLAIM DETAILS.TotalWithdrawal", "visible", true);
                        formClaimInstance.itemOption("CLAIM DETAILS.btnFetchPolicy", "visible", true);
                        formClaimInstance.itemOption("CLAIM DETAILS.LoanPolicies", "visible", true);
                        viewModel.get_loan_first_policy();
                    }
                } else {
            ...
            if (value == "HCI") {
                ...
                if (is_micro == 1) {
                    ...
                    formClaimInstance.itemOption("INSURED DETAILS.isPolicyHolderDeath", "visible", false);
                }
            ...
            if (value == "REF") {//refund
                ...
                formClaimInstance.itemOption("PolicyId", "visible", true);
                formClaimInstance.itemOption("CLAIM DETAILS.PaymentOptions", "visible", true);
                //formClaimSubmitInstance.getEditor("ClaimDefaultPay_method").option("dataSource", PayMethods);
                //formClaimSubmitInstance.itemOption("attachements", "visible", true);
            }
            if (value == "REP") {//REFUND - SUSPENSE ACCOUNT(PROPOSAL)
                vs_rep = true;
                formClaimInstance.itemOption("ProposalNumber", "visible", true);
                PayMethods = SmartLife.Paymentinfo.filter(mode => (mode.IsForClaims == "1" || mode.IsCash == "1"));
                formClaimInstance.itemOption("CLAIM DETAILS.ClaimantName", "visible", true);
                formClaimInstance.itemOption("CLAIM DETAILS.ClaimantMobile", "visible", true);
                formClaimInstance.itemOption("CLAIM DETAILS.id_type", "visible", true);
                ...
                formClaimSubmitInstance.itemOption("attachements", "visible", true);
                formClaimSubmitInstance.itemOption("ViewAttachements", "visible", true);
                //formClaimSubmitInstance.getEditor("ClaimDefaultPay_method").option("dataSource", PayMethods);
                //formClaimSubmitInstance.itemOption("attachements", "visible", true);
            }
            if (value == "RFD") {//WRONGFUL REFUND (FROM BANKS)
                ...
                formClaimSubmitInstance.itemOption("ViewAttachements", "visible", true);
                formClaimSubmitInstance.itemOption("isClaimPayChange", "visible", false);
                formClaimSubmitInstance.itemOption("PaySourceRawData", "visible", false);
                formClaimSubmitInstance.itemOption("PaySourceType", "visible", false);
                formClaimSubmitInstance.itemOption("ReferenceNumber", "visible", true);

                //formClaimSubmitInstance.itemOption("btncomplete", "visible", false);

                //WrongfulBank,ReferenceNumber,WrongfulEmployer,ReferenceNumber,WrongfulDateFrom,WrongfulDateTo
                formClaimSubmitInstance.itemOption("BankCode", "visible", true);
                formClaimSubmitInstance.itemOption("ReferenceNumber", "visible", true);
                formClaimSubmitInstance.itemOption("DateFrom", "visible", false);
                formClaimSubmitInstance.itemOption("DateTo", "visible", false);

                viewModel.getClaimAttachments();
                //viewModel.getPaySourceRawData();
            }
            ...
            if (value == "RFU") {//WRONGFUL REFUND (FROM EMPLOYER)
                ...
                //WrongfulBank,ReferenceNumber,WrongfulEmployer,ReferenceNumber,WrongfulDateFrom,WrongfulDateTo
                formClaimSubmitInstance.itemOption("Emp_code", "visible", true);
                formClaimSubmitInstance.itemOption("ReferenceNumber", "visible", true);
                formClaimSubmitInstance.itemOption("DateFrom", "visible", false);
                formClaimSubmitInstance.itemOption("DateTo", "visible", false);
                //viewModel.getPaySourceRawData();
        ...
        getPaySourceRawData: function (source_type) {

            viewModel.LoadPanelShown(true);
            var get_form = new DB({
                name: "get existing Raw Data"
            });
            let tmp_url = "policy/getBanksRawData?source_type=" + source_type;
            if (claim_type == "RFU") tmp_url = "policy/getPaySourceRawData?source_type=" + source_type;
        ...
        getClaimAttachments: function () {
            //viewModel.LoadPanelShown(true);
            var get_form = new DB({
                name: "get existings checklists"
            });
            get_form.DBget("claims/getClaimAttachments?claim_type=" + claim_type).done(function (result) {
                viewModel.LoadPanelShown(false);
                    ...
                    editorOptions: {
                        ...
                        onValueChanged: function (e) {
                            if (e.value != undefined && e.value != null && e.value != '') {
                                ...
                                if (e.value == "RFD" || e.value == "RFU" || e.value == "REP"  || claim_type == "RFDLOAN" || claim_type == "RFULOAN" || SmartLife.login_type == 1) {
                                    //alert(e.value);
                                    if (SmartLife.login_type == 1) {
                                    ...
                                    if (is_group == 0) {
                                        ...
                                        if (status_code == "10" || status_code == "14") {
                                            
                                            if (e.value == "SUR" && (plan_code == "1" || plan_code == "2")) {
                                                if (is_micro == 0 && e.value == "SUR" && parseInt(prem_units) < 12) {
                                                    formClaimInstance.resetValues();
                                                    DevExpress.ui.dialog.alert("Premium units are less than 12. Cannot Proceed with selected Claim Type", "");
                                                    return;
                                                }
                                            } else {
                                            ...
                                            var MaturityDate = new Date(viewModel.policy_obj()[0].maturity_date);
                                            var currentDate = new Date();
                                            // Compare the selected date with the current date
                                            if (e.value == "MAT") {
                                                if (currentDate < MaturityDate) {
                                                    formClaimInstance.resetValues();
                                                    DevExpress.ui.dialog.alert("Cannot Proceed with Maturity as the Maturity Date is: " + MaturityDate, "");
                                                    return;
                                                } 
                                            }
                                            viewModel.claim_type_changed(e);
                                            ...
                                            if (e.value == "REF" && status_code != "10") {
                                                viewModel.claim_type_changed(e);
                                            } else if ((e.value == "PWD" || e.value == "DTH") && status_code == "15") {
                                                viewModel.claim_type_changed(e);
                                            } else if ((e.value == "SUR" || e.value == "MAT") && status_code == "15") {
                                                if (e.value == "SUR" && (plan_code == "1" || plan_code == "2")) {
                                                    if (is_micro == 0 && e.value == "SUR" && parseInt(prem_units) < 12) {
                                                        formClaimInstance.resetValues();
                                                        DevExpress.ui.dialog.alert("Premium units are less than 12. Cannot Proceed with selected Claim Type", "");
                                                ...
                                                //viewModel.policy_obj()[0].HasFuneralMembers
                                                var MaturityDate = new Date(viewModel.policy_obj()[0].maturity_date);
                                                var currentDate = new Date();
                                                ...
                                                formClaimInstance.resetValues();
                                                DevExpress.ui.dialog.alert("Policy not Established or Lapsed. Cannot Proceed with selected Claim Type", "");
                                                return;
                                            }
                                        }
                        ...
                        editorOptions: {
                            ...
                            /*validationRules: [{
                                ...
                                validationCallback: function (obj) {
                                    ...
                                    let data = $("#dxFormClaimRequest").dxForm('instance').option("formData");
                                    alert(data['dependant']);
                                    if (vs_death && vs_death_dp) {
                                        if (data['dependant'] == '' || data['dependant'] == undefined || data['dependant'] == null) {
                                            return false;
                                        } else {
                                            return true;
                                        }
                            ...
                            editorOptions: {
                                ...
                                columns: [
                                    ...
                                    },
                                    {
                                        ...
                                        dataField: 'ClaimDefaultEftBankaccountNameD',
                                        visible: false
                                    },
                                    {
...
/*, {
                    ...
                    validationRules: [{
                        ...
                        validationRules: [{
                            ...
                            label: {
                                text: "Previous PWD Claim"
                            },
                            editorType: "dxNumberBox",
                            dataField: "TotalWithdrawal",
                            visible: false,
                            editorOptions: {
                                readOnly: true,//change this to true later
                                format: ",##0.##"
                        ...
                        onClick: function (args) {
                            ...
                            if (is_group == 1) {
                                ...
                                if (result.isValid) {
                                    ...
                                    //let data = $("#dxFormClaimRequest").dxForm('instance').option("formData");
                                    console.log(data);
                                    if (claim_type == 'DTH') data['FuneralMembersInfo'] = data['dependant'];
                                    get_life.DBpost("claims/insertClaimEntries", data).done(function (result) {
                                        ...
                                        if (result.success == true) {
                                            ...
                                            //let beneficiaries_arr = [];
                                            let beneficiaries_arr = formClaimInstance.getEditor("beneficiaries");
                                            if (claim_type == "DTH" && beneficiaries_arr != undefined && beneficiaries_arr.length > 0) {
        ...
        getAttachedFiles: function () {
            ...
            get_form.DBget("claims/getClaimFiles?rcd_id=" + rcd_id + "&is_group=" + is_group).done(function (result) {
                viewModel.LoadPanelShown(false);
                if (result.success == true) {
                    //assign datasource to files
        ...
        clear_payment_fields: function () {
            formClaimSubmitInstance.updateData("ClaimDefaultEFTBankBranchCode", '');
            formClaimSubmitInstance.updateData("ClaimDefaultEFTBank_account", '');
            formClaimSubmitInstance.updateData("ClaimDefaultEftBankaccountName", '');
            formClaimSubmitInstance.updateData("ClaimDefaultTelcoCompany", '');
            formClaimSubmitInstance.updateData("ClaimDefaultMobileWallet", '');
        },

        filter_BankBranches: function (val) {
            ClaimBankBranches = SmartLife.BanksBranches.filter(bank => (bank.bank_code == val));
            formClaimSubmitInstance.getEditor("ClaimDefaultEFTBankBranchCode").option("dataSource", ClaimBankBranches);
        },
        ...
        claim_changed_pay_method: function (type, val) {
            ...
            //ClaimDefaultTelcoCompany, ClaimDefaultMobileWallet, ClaimDefaultEFTBank_code, ClaimDefaultEFTBankBranchCode, 
            //ClaimDefaultEFTBank_account, ClaimDefaultEftBankaccountName, 
            if (formClaimSubmitInstance.getEditor("mobile") != null) formClaimSubmitInstance.itemOption("mobile", "visible", false);
            if (formClaimSubmitInstance.getEditor("btnSendOTP") != null) formClaimSubmitInstance.itemOption("btnSendOTP", "visible", false);
            ...
            if (type == 0) {
                ...
                let data_pol = formClaimInstance.option("formData");
                let mobile_no = data_pol['mobile'];
                if (claim_type != "RFD" && claim_type != "RFU" && claim_type != "REP") {
                    formClaimSubmitInstance.updateData("mobile", mobile_no);
                    formClaimSubmitInstance.itemOption("mobile", "visible", true);
                    formClaimSubmitInstance.itemOption("btnSendOTP", "visible", true);
                }
                if (val == "9" || val == "10") {
                    formClaimSubmitInstance.itemOption("ClaimDefaultEFTBankBranchCode", "visible", true);
                    //filter the bank branches...
                    if (val == "9") viewModel.filter_BankBranches('NIB');
                    if (val == "10") viewModel.filter_BankBranches('CBG');
                }
                
                if (val == "7") {//standing order
                ...
                if (val == "9" || val == "10") {//standing order
        ...
        dxFormDClaimSubmit: {
            ...
            items: [
                ...
                ////display the default paymethods..
                {
                    ...
                    /*validationRules: [{
                        type: "custom",
                        message: "Payment Method is required, Kindly put the new Claim Pay Method",
                        validationCallback: function (obj) {
                            //check if IsPep = 1
                            let data = formClaimSubmitInstance.option("formData");
                            if (data["isClaimPayChange"] == 1) {
                                return true;//don't check validation
                            } else {
                                if (obj.value == '' || obj.value == undefined || obj.value == null) {
                                    return false;
                ...
                /////end of display of default paymethods..
                {
                    ...
                    validationRules: [{
                        type: "custom",
                        message: "Payment Method is required, Kindly Select Change Claim Pay Method",
                        validationCallback: function (obj) {
                            //check if IsPep = 1
                            let data = formClaimSubmitInstance.option("formData");
                            if ((obj.value == '' || obj.value == undefined || obj.value == null || obj.value == false) && 
                                    (data["ClaimDefaultPay_methodD"] == null || data["ClaimDefaultPay_methodD"] == undefined
                                        && data["ClaimDefaultPay_methodD"] == '')) {
                                return false;
                            } else {
                                return true;
                            }
                    ...
                    editorOptions: {
                        ...
                        onValueChanged: function (e) {
                            ...
                            if (e.value == 1 || e.value == "1") {
                                let AmountAppliedFor = 0;
                                for (let i = 0; i < PayMethods.length; i++) {
                                    if (PayMethods[i].payment_mode == 1 || PayMethods[i].payment_mode == "1") {
                                        AmountAppliedFor = PayMethods[i].MaximumCashAmountAllowed;
                                    }
                                }

                                //it has to be less then amount requested for
                                let FormData = formClaimInstance.option("formData");
                                if (FormData['AmountAppliedFor'] != undefined && FormData['AmountAppliedFor'] != '') {
                                    if (parseFloat(FormData['AmountAppliedFor']) > AmountAppliedFor) {
                                        DevExpress.ui.dialog.alert("Amount Applied for is greater than maximum allowed of: " + AmountAppliedFor, "ALERT!");
                ...
                //ClaimDefaultEFTBankBranchCode, ClaimDefaultEFTBank_account, ClaimDefaultEftBankaccountName, 
                {
                    ...
                    validationRules: [{
                        type: "custom",
                        message: "Claim Bank is required",
                        validationCallback: function (obj) {
                    ...
                    validationRules: [{
                        ...
                        validationCallback: function (obj) {
                            //check if Doyouhavesecondaryincome = 1
                            let data = formClaimSubmitInstance.option("formData");
                            if (data['payment_mode'] == '7' || data['ClaimDefaultPay_method'] == '9') {
                                if (obj.value == '' || obj.value == undefined || obj.value == null) {
                                    return false;
                                } else {
                                    return true;
                    ...
                    label: {
                        text: "Claim Bank Account Number"
                    },
                    editorType: "dxTextBox",
                    visible: false,
                    dataField: "ClaimDefaultEFTBank_account",
                    validationRules: [{
                        type: "custom",
                        message: "Claim Bank Account Number is required",
                        validationCallback: function (obj) {
                            //check if Doyouhavesecondaryincome = 1
                            let data = formClaimSubmitInstance.option("formData");
                            if (data['ClaimDefaultPay_method'] == '7' || data['ClaimDefaultPay_method'] == '9') {
                    ...
                    validationRules: [{
                        type: "custom",
                        message: "Claim Telco Company is required",
                        validationCallback: function (obj) {
                            //check if Doyouhavesecondaryincome = 1
                            let data = formClaimSubmitInstance.option("formData");
                    ...
                    validationRules: [{
                        type: "custom",
                        message: "Claim MOMO Number is required",
                        validationCallback: function (obj) {
                            //check if Doyouhavesecondaryincome = 1
                            let data = formClaimSubmitInstance.option("formData");
                            if (data['ClaimDefaultPay_method'] == '6') {
                    ...
                    buttonOptions: {
                        ...
                        onClick: function () {
                            ...
                            get_life.DBpost("collections/sendOTP",
                                {
                                    msg: 'Claim Payment Method Changed to MOMO. ',
                                    mobile_no: mobile_no
                ...
                ////////////////////Loan Repayment///////////////////////////
                {
                    ...
                    validationRules: [{
                        ...
                        validationCallback: function (obj) {
                            //check if Doyouhavesecondaryincome = 1
                            let data = formClaimSubmitInstance.option("formData");
                            if (data['LoanDefaultPay_method'] == '2') {
                                if (obj.value == '' || obj.value == undefined || obj.value == null) {
                                    return false;
                                } else {
                                    return true;
...

                {
                    ...
                    editorOptions: {
                        ...
                        onValueChanged: function (e) {
                            //filter the bank branches ClaimBankBranches
                            formClaimSubmitInstance.updateData("LoanDefaultEFTBankBranchCode", '');
                            LoanBankBranches = SmartLife.BanksBranches.filter(bank => (bank.bank_code == e.value));
                            formClaimSubmitInstance.getEditor("LoanDefaultEFTBankBranchCode").option("dataSource", LoanBankBranches);
                        }
                    },
                    validationRules: [{
                        type: "custom",
                        message: "Loan Bank is required",
                        validationCallback: function (obj) {
                            //check if Doyouhavesecondaryincome = 1
                            let data = formClaimSubmitInstance.option("formData");
                            if (data['LoanDefaultPay_method'] == '7' || data['LoanDefaultPay_method'] == '9') {
                                if (obj.value == '' || obj.value == undefined || obj.value == null) {
                                    return false;
                                } else {
                                    return true;
                    ...
                    validationRules: [{
                        type: "custom",
                        message: "Loan Bank Account Number is required",
                        validationCallback: function (obj) {
                            //check if Doyouhavesecondaryincome = 1
                            let data = formClaimSubmitInstance.option("formData");
                            if (data['LoanDefaultPay_method'] == '7' || data['LoanDefaultPay_method'] == '9') {
                    ...
                    validationRules: [{
                        type: "custom",
                        message: "Loan Telco Company is required",
                        validationCallback: function (obj) {
                            //check if Doyouhavesecondaryincome = 1
                            let data = formClaimSubmitInstance.option("formData");
                            if (data['LoanDefaultPay_method'] == '6') {
                                if (obj.value == '' || obj.value == undefined || obj.value == null) {
                                    return false;
                                } else {
                                    return true;
                    ...
                    validationRules: [{
                        type: "custom",
                        message: "Loan MOMO Number is required",
                        validationCallback: function (obj) {
                            //check if Doyouhavesecondaryincome = 1
                            let data = formClaimSubmitInstance.option("formData");
                            if (data['LoanDefaultPay_method'] == '6') {
                ...
                , {
                    ...
                    validationRules: [{
                        type: "custom",
                        message: "WrongfulEmployer is required",
                        validationCallback: function (obj) {
                            //check if Doyouhavesecondaryincome = 1
                            if (claim_type == "RFU" || claim_type == "RFULOAN") {
                                if (obj.value == '' || obj.value == undefined || obj.value == null) {
                                    return false;
                                } else {
                                    return true;
                    ...
                    colSpan: 2,
                    label: {
                        text: "Reference Number / Staff No"
                    },
                    editorType: "dxTextBox",
                    visible: false,
                    editorOptions: {
                    ...
                    validationRules: [{
                        ...
                        validationCallback: function (obj) {
                            if (claim_type == "RFD" || claim_type == "RFU" || claim_type == "RFULOAN") {
                                if (obj.value == '' || obj.value == undefined || obj.value == null) {
                                    return false;
                                } else {
                                    return true;
                    ...
                    validationRules: [{
                        type: "custom",
                        message: "Payment Date From is required",
                        validationCallback: function (obj) {
                    ...
                    validationRules: [{
                        type: "custom",
                        message: "Payment Date To is required",
                        validationCallback: function (obj) {
                            if (claim_type == "RFD" || claim_type == "RFU") {
                                if (obj.value == '' || obj.value == undefined || obj.value == null) {
                                    return false;
                    ...
                    editorOptions: {
                        ...
                        onRowClick: function (e) {
                            if (window.location.host == "192.168.1.24:90") {
                                window.open('http://192.168.1.24:90/SmartLifeDocuments/ClaimsDocuments/' + e.data.Description, '_blank', 'hidden=no');
                            } else {
                                window.open('http://197.159.142.171:90/SmartLifeDocuments/ClaimsDocuments/' + e.data.Description, '_blank', 'hidden=no');
                            }
                        },
                        columns: [
                            {
                    ...
                    buttonOptions: {
                        ...
                        onClick: function (args) {
                            ...
                            if (is_group == 1) {
                                ...
                                //validate claim payment methods
                                let data = formClaimInstance.option("formData");
                                if (claim_type != "DTH" && data['isClaimPayChange'] == 0 && (data['ClaimDefaultPay_methodD'] == undefined || data['ClaimDefaultPay_methodD'] == '')) {
                                    DevExpress.ui.dialog.alert("Kindly Change / Add Claim Payment Method", "Payment Method Missing!");
                                    return;
                                } else {
                                
                                    if (claim_type == "DTH") {
                                        if (data['dependant'] != undefined && data['dependant'] != '') {
                                            var Dependant = DependantsStore.filter(dependant => (dependant.id == data['dependant']));
                                            if (Dependant[0]['Relationship'] != 'SF' && data['isClaimPayChange'] == 0 && (data['ClaimDefaultPay_methodD'] == undefined || data['ClaimDefaultPay_methodD'] == '')) {
                                                DevExpress.ui.dialog.alert("Kindly Change / Add Claim Payment Method", "Payment Method Missing!");
                                                return;
                                            }
                                        }
                                    ...
                                    if (viewModel.validate_attachments()) {
                                        ...
                                        if (claim_type == "RFD" || claim_type == "RFU" || claim_type == "RFDLOAN" || claim_type == "RFULOAN") {
                                            let dataSubmit = formClaimSubmitInstance.option("formData");
                                            delete dataSubmit.PaySourceRawData;
                                            delete dataSubmit.PaySourceType;
                                        }
                                        ...
                                        if (rcd_id != undefined && parseInt(rcd_id.toString()) > 0) {
                                            data['id'] = rcd_id;
                                        }
                                        data['user_id'] = SmartLife.pos_name;
                                        if (data['event_date'] != undefined && data['event_date'] != '' && data['event_date'] != null) data['event_date'] = viewModel.formatDates(new Date(data['event_date']));
                                        ...
                                        if (result.isValid ) {
                                            if (claim_type != "RFD" && claim_type != "RFU" && claim_type != "RFDLOAN" && claim_type != "RFULOAN" &&
                                                 isClaimPayChange == 1 && is_send_OTP) {
                                                ...
                                                if (mobile_no == undefined || mobile_no == "") {
                                                    ...
                                                    get_life.DBpost("collections/sendOTP",
                                                        {
                                                            msg: 'Claim Payment Method Changed. ',
                                                            mobile_no: mobile_no
                                                        }).done(function (result) {
                                                            viewModel.LoadPanelShown(false);
                                                            if (result.success == true) {
            ...
            get_form.DBget(url).done(function (result) {
                ...
                if (result.success) {
                    ...
                    if (isEmptyString(LoanPolicies)) {
                        ...
                        formClaimInstance.updateData("TotalLoanValue", TotalLoanAmount);
                        formClaimInstance.updateData("CurrentCashValue", AmountAvailable);

                        formClaimInstance.updateData("PreviousloanAmount", PreviousloanAmount);
                        formClaimInstance.updateData("TotalLoanAvailable", TotalLoanAvailable);
                        formClaimInstance.updateData("TotalWithdrawal", TotalWithdrawal);
                    }
                    //viewModel.hide_policies();
                    formClaimInstance.updateData("LoanPolicies", LoanPolicies);
                } else {
                    viewModel.show_test(result.message, 'error');
                }
            }).fail(function () {
            ...
            get_form.DBget(url).done(function (result) {
                ...
                if (result.success) {
                    ...
                    if (isEmptyString(LoanPolicies)) {
                        ...
                        formClaimInstance.updateData("TotalLoanValue", tmp_TotalLoanAmount);
                        formClaimInstance.updateData("CurrentCashValue", tmp_amount_available);

                        formClaimInstance.updateData("PreviousloanAmount", tmp_PreviousloanAmount);
                        formClaimInstance.updateData("TotalLoanAvailable", tmp_TotalLoanAvailable);
                        formClaimInstance.updateData("TotalWithdrawal", tmp_TotalWithdrawal);
                    }
                    viewModel.hide_policies();
                    formClaimInstance.updateData("LoanPolicies", LoanPolicies);
        ...
        show_beneficiary: function (e) {
            ...
            let tmp_values = {
                ...
                isClaimPayChange: e.data.isClaimPayChange,
        ...
        dxFormBeneficiary: {
            ...
            items: [
                ...
                //ClaimDefaultEftBankaccountName
                {
                    ...
                    /*validationRules: [{
                        type: "custom",
                        message: "Payment Method is required, Kindly put the new Claim PayMethod",
                        validationCallback: function (obj) {
                            //check if IsPep = 1
                            let data = formBeneficiaryInstance.option("formData");
                            if (data["isClaimPayChange"] == 1) {
                    ...
                    editorOptions: {
                        onValueChanged: function (e) {
                            ...
                            */
                            ...
                            if (e.value == 1) {
                                formBeneficiaryInstance.itemOption("ClaimDefaultPay_method", "visible", true);
                                formBeneficiaryInstance.getEditor("ClaimDefaultPay_method").option("dataSource", PayMethods);
                            } else {
                                formBeneficiaryInstance.itemOption("ClaimDefaultPay_method", "visible", false);
                            }
                            viewModel.refresh_dataSources();
                ...
                //ClaimDefaultEFTBankBranchCode, ClaimDefaultEFTBank_account, ClaimDefaultEftBankaccountName, 
                {
                    ...
                    editorOptions: {
                        ...
                        onValueChanged: function (e) {
                            //filter the bank branches ClaimBankBranches
                            formBeneficiaryInstance.updateData("ClaimDefaultEFTBankBranchCode", '');
                            ClaimBenefBankBranches = SmartLife.BanksBranches.filter(bank => (bank.bank_code == e.value));
                            formBeneficiaryInstance.getEditor("ClaimDefaultEFTBankBranchCode").option("dataSource", ClaimBenefBankBranches);
                        }
                    },
                    validationRules: [{
                        type: "custom",
                        ...
                        validationCallback: function (obj) {
                            //check if Doyouhavesecondaryincome = 1
                            let data = formBeneficiaryInstance.option("formData");
                            if (data['ClaimDefaultPay_method'] == '9' || data['ClaimDefaultPay_method'] == '10') {
                                if (obj.value == '' || obj.value == undefined || obj.value == null) {
                                    return false;
                                } else {
                                    return true;
                    ...
                    editorOptions: {
                        closeOnOutsideClick: true,
                        dataSource: ClaimBenefBankBranches,
                        displayExpr: 'bankBranchName',
                        valueExpr: 'id'
                    },
                    ...
                    validationRules: [{
                        type: "custom",
                        message: "Claim Bank Account is required",
                        validationCallback: function (obj) {
                            //check if Doyouhavesecondaryincome = 1
                            let data = formBeneficiaryInstance.option("formData");
                            if (data['ClaimDefaultPay_method'] == '7' || data['ClaimDefaultPay_method'] == '9') {
