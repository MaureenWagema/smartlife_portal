    ...
    var viewModel = {
        ...
        viewShown: function () {
            ...
            if (viewModel.applications_store().length == 0) {
                ...
                let default_url = "policy/getPolicyDetails?IsCreditLifeUser=" + SmartLife.IsCreditLifeUser;
                if (SmartLife.pos_type == 2) default_url = "policy/getMicroPolicyDetails";
                viewModel.get_policy_applications(default_url);
            }

            if (is_micro == 0) viewModel.vs_command(true);
        },
        ...
        to_paysource_data: function () {
            //viewModel.navigateForward("PaySourceData", "");
            var data = { claim_type: 'RFU' };
            viewModel.navigateForward("PaySourceData", JSON.stringify(data));
        },

        to_bank_data: function () {
            var data = { claim_type: 'RFD' };
            viewModel.navigateForward("PaySourceData", JSON.stringify(data));
        },
