SmartLife.application_form = function (params) {
    ...
    var viewModel = {
        ...
        display_vs_mortgage: function () {
            ...
            ClaimAttachDataSource = [{ id: 'facility_letter', name: "FACILITY LETTER", IsMandatory: 1 }, { id: 'medical_rpt', name: "MEDICAL FORM", IsMandatory: 1 }];
        ...
        refresh_dynamic_ds: function () {
            ...
            //regions
            if (formPolicyDetailsInstance.getEditor("region") != null)
                formPolicyDetailsInstance.getEditor("region").option("dataSource", Regions);
            //claim_bank branches
        ...
        ComputePremiumCreditLife: function (plan_code, anb, term, paymode, sa) {
            viewModel.LoadPanelShown(true);
            let calc = new DB({
                name: "calculating credit life premium"
            });
            let data = {
                'plan_code': plan_code,
                'anb': anb,
            ...
            calc.DBpost("calc/ComputePremiumCreditLife", data).done(function (result) {
                viewModel.LoadPanelShown(false);
                if (result.success == true) {
                    console.log(result);
                    //assign all here
                    //sum_assured,policy_fee,inv_prem,rider_prem,transfer_charge,riders
                    formPolicyDetailsInstance.updateData("total_premium", result.premium);
                    //formInstance.updateData("policy_fee", result.policy_fee);
                    //formPolicyDetailsInstance.updateData("inv_premium", result.inv_prem);
                    //formPolicyDetailsInstance.updateData("rider_premium", result.rider_prem);
                    //console.log(result.riders);
                ...
                },
                {
                    ...
                    editorOptions: {
                        ...
                        onValueChanged: function (e) {
                            ...
                            if (e.value == "001") {
                                //alert("here");
                                Regions = SmartLife.Regions;
                                //formPolicyDetailsInstance.getEditor("region").option("dataSource", Regions);
                                formPersonalDetailsInstance.getEditor("region").option("dataSource", Regions);
                                //viewModel.refresh_dynamic_ds();
                            } else {
                                Regions = [];
                                formPersonalDetailsInstance.getEditor("region").option("dataSource", Regions);
            ...
            items: [
                 {
                    ...
                    editorOptions: {
                        onValueChanged: function (e) {
                            ...
                            let data = formPolicyDetailsInstance.option("formData");
                            let plan_code = data['plan_code'];
                            let anb = data_personal['anb'];
                            let term = data['term'];
                            let paymode = data['paymode_code'];
                            let sa = e.value;
                            viewModel.ComputePremiumCreditLife(plan_code, anb, term, paymode, sa);
                        }
                    },
                    validationRules: [{
                        type: "required",
                    ...
                    editorOptions: {
                        ...
                        onRowClick: function (e) {
                            if (window.location.host == "192.168.1.16:90") {
                                window.open('http://192.168.1.16:88/PolicyDocuments/' + e.data.Description, '_blank', 'hidden=no');
                            } else {
                                window.open('http://197.159.142.171:88/PolicyDocuments/' + e.data.Description, '_blank', 'hidden=no');
                            }
                        },
                        columns: [
                            {
